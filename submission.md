...
```text
6~12명의 NPC가 보여 주는 행동의 원인과 결과를 관찰하세요. 대부분이 따르는 단 하나의 진짜 규칙과, 그 규칙만 어기는 한 명을 두 번의 기회 안에 찾는 3D 관찰 추리 게임입니다. 3단계 튜토리얼로 진짜 규칙, 가짜 규칙과 종소리 교란을 익히고, 설치·로그인 없이 모바일과 데스크톱 브라우저에서 바로 플레이할 수 있습니다.
```
...
```text
처음 접속하면 3단계 튜토리얼에서 진짜 규칙, 가짜 규칙과 종소리 교란을 순서대로 익힐 수 있습니다. 핵심 플레이를 빠르게 확인하려면 일반 게임의 6명 모드를 선택하세요. 화면의 규칙 후보를 읽고 NPC가 특정 조건에서 어떤 행동을 하는지 관찰한 뒤, 대부분이 지키는 규칙 하나와 그 규칙만 어기는 NPC를 찾으면 됩니다. 기회는 두 번이며 첫 플레이에는 현재 기기에 맞는 조작 안내가 표시됩니다.
```
...
Codex는 TypeScript와 Three.js 기반의 NPC·카메라·행동 규칙 시스템, 데스크톱과 모바일 입력, Web Audio 사운드, 점수·공유·Firebase 리더보드, 반응형 UI와 GitHub Pages 배포를 구현했습니다. 특히 10개 규칙에서 참가 인원에 맞는 후보를 고르고, 정답 규칙만 N-1:1로 배정하면서 가짜 규칙은 반반으로 유지하는 라운드 생성기를 만들었습니다. 진짜 규칙만 관찰하는 단계부터 가짜 규칙과 종소리 교란까지 차례로 배우는 3단계 튜토리얼도 추가했습니다.
...
게임 실행 중에는 생성형 AI나 OpenAI API를 호출하지 않습니다. NPC 규칙과 행동 시뮬레이션은 브라우저에서 로컬로 생성·실행되며, Firebase는 선택형 랭크 최고 점수 저장에만 사용됩니다. 프로젝트는 챌린지 기간인 2026년 8월 16일에 처음 생성되어 전체 개발이 기간 안에 진행되었습니다.
```
...
```text
Codex와 함께 TypeScript·Three.js 기반 NPC, 10개 행동 규칙 생성기, 3단계 튜토리얼, 데스크톱·모바일 카메라, 반응형 HUD, Web Audio, 점수·공유·리더보드와 GitHub Pages 배포를 구현했습니다. 모바일 고발 카메라, Pointer Lock 커서, 행동 판독성 같은 실제 QA 문제도 반복 수정했습니다. 사람은 핵심 추리 규칙, 난이도, 종 교란, 아트·UI 방향을 결정했고 Codex는 이를 코드·테스트·문서·Git 기록으로 구체화했습니다. 런타임 생성형 AI 호출은 없습니다.
```
...
| 0:12~0:30 | 3단계 튜토리얼 선택과 단계 화면 | 진짜 규칙 → 가짜 규칙 → 종소리 교란을 실제 플레이로 익히는 구조를 보여 준다. |
| 0:30~0:42 | 6/9/12 일반 게임과 랭크 도전 | 설치·로그인 없이 브라우저에서 바로 실행되며 모바일과 데스크톱을 지원한다고 설명한다. |
| 0:42~0:58 | 규칙 안내와 규칙 후보 HUD | 조건이 생기면 행동이 나온다는 구조, 진짜 규칙은 N-1:1이고 나머지는 가짜 단서라고 설명한다. |
| 0:58~1:28 | NPC 이동과 서로 다른 행동 | 빨간 옷 접근, 중앙 진입, 서로 마주보기 등 원인과 Jump·Wave·Spin 같은 결과를 함께 보여 준다. |
| 1:28~1:40 | 종소리 교란 | 일부 NPC에게 행동을 강제하지만 정답 규칙은 아니라는 점을 짧게 보여 준다. |
| 1:40~1:57 | 규칙/NPC 마킹과 FOLLOW | 관찰 중 가설을 기록하고 의심 NPC를 따라갈 수 있음을 보여 준다. |
| 1:57~2:18 | 고발과 정답 공개 | NPC를 지목하고 클로즈업 판정, 성공 또는 마지막 오답 뒤 실제 Odd NPC 공개를 보여 준다. |
| 2:18~2:32 | 점수와 공유 결과 | 일반 점수, 3단계 랭크 도전, 결과 공유 기능을 빠르게 보여 준다. |
| 2:32~2:52 | 코드·Git 기록을 짧게 삽입 | Codex로 규칙 생성, 튜토리얼, 모바일 카메라, UI, 사운드와 배포 문제를 반복 해결했다고 설명한다. |
...
| 심사 기준 | 제출에서 강조할 내용 |
| --- | --- |
| Playability | URL 접속 즉시 플레이, 진짜·가짜·종 교란을 익히는 3단계 튜토리얼, 6명 모드의 빠른 진입, 모바일·데스크톱 대응 |
...
- [ ] 기획서의 최소 10px 원칙과 다른 튜토리얼 HUD 단계 라벨 `9px`을 10px로 조정하거나 예외로 확정한다.
...
| 심사위원용 플레이 안내 | 준비 완료 |
| 3단계 튜토리얼 소개 | 제출 본문·영상 구성안에 반영 완료 |
| 3분 데모 영상 | 구성안 준비 완료 · 촬영과 업로드 필요 |
| 개인정보·팀명·연락처 | 사용자 입력 필요 |
| 최종 신청서 제출 | 사용자 검토와 약관 동의 후 진행 |

## 9. 2026-08-24 내부 정합성 점검

- `theoddone.md`의 싱글플레이 범위, 6/9/12명 일반 모드, 6→9→12 랭크 코스, 행동 규칙 10개, 고발 기회 2회, 점수와 Firebase 최고 기록 구조가 현재 코드·설정과 일치한다.
- 멀티플레이는 기획서의 제외 범위이며 코드와 제출 자료에도 포함하지 않는다.
- `npm run build`가 성공했고 GitHub Pages 공개 URL에서 시작 화면, 튜토리얼 1/3 진입과 Firestore 최고 점수 화면이 오류 로그 없이 로드되는 것을 확인했다.
- 제출을 막는 오류는 발견되지 않았다. 남은 문서·코드 불일치는 튜토리얼 HUD 단계 라벨 한 곳의 `9px`뿐이며, 기획서의 최소 `10px` 원칙과 맞추려면 제출 전에 조정 또는 예외 확정이 필요하다.
